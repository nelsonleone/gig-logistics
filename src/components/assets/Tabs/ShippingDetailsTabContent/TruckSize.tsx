...
export default function TruckSize(props:IProps){
    ...
    return(
        <Modal open={props.open} onClose={props.onClose} id="shipnow_trucksize_modal">
            <div className="w-[90%]  overflow-y-auto h-[30em] top-0 bottom-0 absolute left-0 m-auto md:max-w-[30em] md:w-[30em] right-0 rounded-lg  shadow-xl bg-[#FFFFFF] text-primary px-8 pt-14 pb-8 text-center">
                <IconButton aria-controls="shipnow_trucksize_modal" onClick={props.onClose} aria-expanded={props.open ? "true" : "false"} className="absolute top-4 left-4 text-primary">
                    <IoMdClose />
                </IconButton>
