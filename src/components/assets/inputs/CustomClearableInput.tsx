    ...
    return(
        <div className={`bg-gray-100 relative min-h-[3.2em] } w-full rounded-lg overflow-hidden ${containerStyles}`}>
            <input 
               ...
               value={value} 
               id={id}
               className={`focus:border focus:border-cyan-700 rounded-lg focus:outline-none focus:outline-0  absolute w-full h-full top-0 bottom-0 left-0 right-0 m-auto py-6 px-4  ${inputStyles}`}
            />
            {
                value &&
