...
export default async function About(){
    ...
    return(
        <main className="page text-primary px-6 xl:px-10">
            <Suspense fallback={<CustomSkeleton variant="rectangular" className="w-full h-28 lg:w-1/2 lg:mx-auto" />}>
                ...
                <p className="text-center font-base mb-4">{aboutPageContentData?.introText}</p>
            </Suspense>

            <MainAboutUsPageContent
               aboutPageContentData={aboutPageContentData} 
               className="flex flex-col gap-8 mt-8 lg:mt-16 lg:flex-row lg:justify-between lg:items-start lg:gap-16" 
            />

            <section className="lg:flex gap-8 justify-between items-center md:my-10">
                {
                    dmArr.length ? dmArr.map((val,index) => (
