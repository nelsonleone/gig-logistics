...
export default function Error({
  ...
  return (
    <div className="page relative flex flex-col justify-center items-center bg-base-color2">
      ...
      <div className="base-color1 z-20">
        ...
        <div className="flex justify-between lg:justify-center gap-8 mt-4">
          <button className={`${inter.className} bg-accent-color text-base-color1 p-3 text-center rounded cursor-pointer`} onClick={() => reset()}>Try Again</button>
          <button className={`${inter.className} bg-accent-color text-base-color1 p-3 text-center rounded cursor-pointer`} onClick={() => router.push("/")}>Go To Homepage</button>
        </div>
      </div>
    </div>
  )   
